...
statsRoutes.get("/", async (c) => {
  ...
  const [stats] = await pool.query(
    `SELECT
        ...
        COALESCE(ROUND(
            SUM(TIMESTAMPDIFF(MINUTE, b.start_time, b.end_time)) / 60.0
        , 1), 0) as total_hours
     FROM classroom c
  ...
  const [totalBookings] = await pool.query("SELECT COUNT(*) as c FROM booking");
  const [totalClassrooms] = await pool.query("SELECT COUNT(*) as c FROM classroom");
  const [totalUsers] = await pool.query("SELECT COUNT(*) as c FROM user_or_group");

  const maxHours = 40;

  const statsRows = (stats as any[])
    .map((s) => {
      const pct = Math.min(Math.round((Number(s.total_hours) / maxHours) * 100), 100);
      return `
      <tr>
  ...
  const content = `
    <div class="stats-grid">
      <div class="card stat-card">
        <div class="number">${(totalBookings as any[])[0].c}</div>
        <div class="label">Broneeringuid kokku</div>
      </div>
      <div class="card stat-card">
        <div class="number">${(totalClassrooms as any[])[0].c}</div>
        <div class="label">Arvutiklasse</div>
      </div>
      <div class="card stat-card">
        <div class="number">${(totalUsers as any[])[0].c}</div>
